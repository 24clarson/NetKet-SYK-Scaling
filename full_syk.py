...
# Set up full SYK Hamiltonian (optional complex coefficients)
np.random.seed(0)
J = np.random.normal(0, 1, (N, N, N, N)) # + 1j*np.random.normal(0, 1, (N, N, N, N))
J /= N
for i in range(N):
    for j in range(N):
        for k in range(N):
        ...
        for k in range(N):
            for l in range(N):
                if len(set([i,j,k,l])) != 4:
                    continue
                hamiltonian += J[i][j][k][l] * cdag(hi, i) * cdag(hi, j) * c(hi, k) * c(hi, l)
